...
def read_english_text(file_index, mappings):
	...
	for mapping in sorted(mappings, key=lambda mapping: mapping['en_file']):
		...
		try:
			line_id, _, line_text = line.rstrip().split('\t', maxsplit=3)
		except:
...
def main():
	...
	read_english_text(file_index, mappings)

	columns = ['foreign_id', 'foreign_family', 'en_id', 'en_family', 'en_text']
	...
	for filename in sys.argv[2:]:
		...
		read_foreign_text(filename, mappings, column=int(column) - 1, dest=filename)
